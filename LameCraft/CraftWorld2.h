...
typedef struct player
{
	...
	int walkSpeed;
	int fireResistance;
	int air_time;	//Air you can breathe underwater till you start to drowned
	Vector3 oldspawn;
	bool safe_spawn;	//Player won't take any damage on falling spawn
	float objective_time[5];	//Just a general use timer for things
	int active_heart;
	int active_hunger;
...
player_t player;
    ...
    bool PlayerTouchEndMap(Vector3 pos);
	...
	bool PlayerOnLadder(Vector3 pos);
	bool PlayerTouchCactus(Vector3 PlayerPos);
	bool PlayerTouchEndermanBody(Vector3 PlayerPos);
	bool PlayerTouchEndermanLegs(Vector3 PlayerPos);
	bool PlayerTouchZombieBody(Vector3 PlayerPos);
	bool PlayerTouchZombieLegs(Vector3 PlayerPos);
	bool PlayerOnHalfBlock(Vector3 PlayerPos);
	bool PlayerInWeb(Vector3 PlayerPos);
	bool PlayerOnSoulSand(Vector3 PlayerPos);
	bool PlayerOnLilyPad(Vector3 PlayerPos);
	...
	void SpawnMobs(int id);
	void RemoveMobs(int id);

	void UpdateAnimalPos(int id);
	void UpdateMobPos(int id);
